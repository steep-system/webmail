...
CD.func = {
	...
	SavePos : function(id) {
		...
		if (-1 == userAgent.indexOf("IE") && (userAgent.indexOf("Firefox") > -1 || userAgent.indexOf("Mozilla") > -1)){
	...
	LoadPos : function(id){
		if(CD.g[id].range){
			if (CD.browser) {
				...
				CD.g[id].selection.removeAllRanges();
				CD.g[id].selection.addRange(CD.g[id].range);
			}
		}
	},
	fHide : function(id,k){
	...
	toolprompt : function(id,k,t){
		...
		switch(t){
			...
			d += "<iframe frameborder='0' scrolling='no' src='showuploadinnerimg?mid="+$('#mid').val()+"' style='width:260px;height:25px;'></iframe>";
			d += "</div>";
			...
			d += "<input id='"+id+"_link_"+t+"' para='InsertImage' type='button' value='确定' style='margin-left:2px;width:70px;'>";
